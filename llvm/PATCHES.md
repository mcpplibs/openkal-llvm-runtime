...
| 守卫 | 处数 | 怎么解决的 |
|---|---|---|
| `_LIBCPP_WIN32API` | **12** | 已 `port/include/__config` 撤回了它 → **自己落到 POSIX 分支** |
| `__SEH__` / `_LIBUNWIND_SUPPORT_DWARF_UNWIND` | 2 | 已 `-fdwarf-exceptions`,异常机制跟随我们带的 unwinder |
| 裸 `_WIN32` | 3 | 已 三处全换(AddressSpace / RWMutex / UnwindCursor) |
| `emutls.c` 的 `_WIN32` | 2 | 已 走 POSIX 分支 |
